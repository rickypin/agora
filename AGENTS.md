...
- 并行施工（多个 agent 同时开工时；第一波编排见各任务 notes 的「文件归属 / 接缝」）：一任务一 worktree（`git worktree add ../agora-wt/<id> -b <id> main`，与 `worktree_root` 约定一致；beads 库经 git common dir 自动共享，worktree 里直接用 `bd`），提交前 `git rebase main`，切片小到一次 rebase 解决得了；手工起 daemon 用独立 `AGORA_HOME` 与 `server.listen` 端口，别碰开发机上真的 agora；热点文件规矩：`docs/spec/api.md` 只在自己端点的小节里追加，侧栏行 / Header / New Agent 对话框谁先碰谁先把那块抽成独立组件（抽取单独一个 commit、不改行为、立刻 push），后来者只改新组件；要改别人归属的文件先看对方分支有没有未合入的改动。
