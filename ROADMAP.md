...
演示剧本：1. 隔离 daemon + fake-agent 起一个无句柄 external 会话，只发 SessionStart：10 s 内行变 turn done「awaiting first prompt」。2a. daemon 运行中 kill 掉隔离 daemon 的 tmux server：下一 tick agora 行显示 finished「runtime server gone」并弹一条通知（RUNNING → FINISHED）。2b. server 不在时起 daemon：行同样 finished，不弹通知（Differ 第一轮只建基线），终端面板不给「重新连接」而给「运行时会话已不在」+ Restart / 删除。3. 起 daemon 前往 inbox 塞一个 PermissionRequest，重放中再塞一个 Stop：daemon 起来 10 s 内两条都被消费，行不钉在 waiting。4. 👁 Mac 上 codex desktop 子对话不再以普通行出现（按决策）。5. GET /api/sessions 里每一行的 process 字段是 alive|gone|unknown 三值，finished 行不再 alive。
