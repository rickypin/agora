...
| Detach / 关闭终端视图 | 关掉那条 `attach` 流 | 运行时不知道浏览器存在 |
...
| daemon 重启 | `list()` ⨝ SQLite | 已知 ref 活着 → 重建；已知 ref 已死 → 按 `exit` 报 FINISHED / FAILED；**已知 ref 不在 → `ended_at` 补今天（近似）、状态 FINISHED**（source process、conf 0.8、reason `runtime session gone`，分 server gone / session gone），可 Delete 或 Restart（此时 Restart 退化为同名 `create`，无 scrollback）；未知 ref 在 agora socket → 未注册（多半是库丢了）；未知 ref 在默认 socket → 可采纳的未注册会话（采纳后 `origin = adopted`，§5.5）。"不在"曾经是 UNKNOWN（reason `runtime session missing`，2026-09-03 写进本表），**2026-09-19 据 agora-u5p 改判**：会话连同 pane 一起没了 ⇒ pane 进程收 SIGHUP ⇒ agent 确定不在，这是事实不是"看不清"（MISSION §4.3），UNKNOWN 又没有任何出口（Mac 2026-09-18：daemon 重启后 tmux 无 server，每一行钉在 ? 35m，主区给一个永远连不上的「重新连接」；其中两行库里有 killed_at / ended_at、状态说看不清，同一行自相矛盾） |
| **运行中 server 没了** | 下一 tick `list()` 返回**空列表且不报错**（没有会话不是故障），每行各自 connect 不上自己的 socket | 与 daemon 重启同一个结论：这一批行各自 FINISHED（reason 里是 `server gone`），并当场补 `ended_at`（近似）——`reconcile` 只在启动时跑一次，运行中死掉的等不到下一次重启。通知照 process gone 发一次；求差器第一轮只建基线，所以**重启时发现** server 不在的那一批不弹通知，只有运行中死的弹（agora-u5p notes ④） |
...
**"运行时会话不在"为什么只给 0.8 不给 1.0（agora-u5p，2026-09-19）**："server 在不在"的判据是结构事实——能不能 connect 上那个 unix socket（与 `list_socket` 同一条，不匹配 stderr 文本，规则 10）。它有一个已知的**假阳性**：socket 文件被 `systemd-tmpfiles` / `tmpreap` 之类扫掉而 server 还活着（tmux 靠 SIGUSR1 重建 socket，client 却说 "no server running"）。那一刻每一行都会被读成"会话没了"。还有第二条同形状的：`list()` 对采纳来的 socket 的失败是吞掉的（不能拖垮 agora 自己的会话列表），于是"列表里没有"可能只是"那次根本没看那个 socket"。两条都不能 100% 排除，所以这条结论的把握是 0.8（**2026-09-22 更新，agora-dkv3**：第二条已经排除——`Runtime::list()` 交出 per-socket 的扫描结果，没扫成的那个 socket 上的行落 UNKNOWN `runtime unavailable: socket <名字>: …`，不判 `runtime_gone`、不写 `ended_at`，两条 missing 分支（`view` 每 tick、`reconcile` 启动时）共用这道门；0.8 现在只为第一条定价）：它盖不过 hook 先说的结束（同分，`Machine::process_fact_is_no_better`），也不去动 `unknown_signal_grace` / 退出码那套已经确定的事；并且它不写准确时刻，`ended_at` 带 `ended_at_approximate`。拿不准的是"哪一种没了"，不是"还在不在"——pane 没了 agent 就不在，这一条不打折。反例（写下来防被好心改回 UNKNOWN 或改回 1.0，2026-09-19 实测）：把 conf 抬到 1.0，`tests/state_machine.rs::runtime_session_gone_finishes_the_row_instead_of_pin_it_at_unknown` 第一段的 conf 断言先红（`tests/session_manager.rs` 另有两条钉住 0.8）；第一段 panic 后第四段轮不到执行，所以那一句单独验过：拿掉第一段断言后同一份 1.0 让第四段也红（`source` 从 hook 被抢成 process，即 `process_fact_is_no_better` 为假，宿主 `session ended (hook)` 的说法会被进程事实抢掉）。把结论改回 UNKNOWN 则红 10 条（`tests/session_manager.rs` 6 条 + `tests/state_machine.rs` / `tests/session_tmux.rs` 各 1 条 + `tests/status_truth_table.rs` 2 条；2026-09-19 在进程三态（agora-5gg.18）之后重测过一遍。同一轮里 `tests/invariants_peer.rs::invariant_11_unauthenticated_peer_and_device_rejected` 也红了，那是它自己的 10 s 等待超时（等「再签后在线」），与这条结论无关）。第四段那条「同状态同分不盖 hook 先说的结束」另有一枚独立的钉子：`tests/status_truth_table.rs::runtime_session_gone_does_not_outvote_a_hook_that_spoke_first`——同一份 conf 1.0 让它单独红，不必先把第一段的断言拿掉才轮到它（2026-09-19 实测）。

**"运行时会话没了"的出口在 UI 上是什么**：这一行没有 pane 可连，终端那一格不能再给一个「重新连接」（Mac 现场：按下去就是一次 `no sessions` + 退出码 1）。给的是「运行时会话已不在」+ Restart（就是上表那一行的"退化为同名 `create`"）与删除记录，实现在 `web/src/TerminalView.tsx`；判据（`status=finished` 且 reason 含 `runtime session gone`）在 `web/src/terminal.ts::runtimeSessionGone`。
...
- **tmux ≥ 3.2**：`new-session -e`（3.2）、`window-size latest`（3.1）、`respawn-pane -e`（3.0）。`pane_dead_signal` / `pane_dead_time` 是 3.3 才有的（ubuntu-22.04 CI 的 3.2a 实测两者皆空，2026-09-03）：3.2a 上被信号杀死的 pane 报 `Exit::Signal("unknown")`，`exited_at` 取运行时首次观测到死亡的时刻；pane 死后 `pane_dead_status` 为空既可能是退出码尚未收集（pty EOF 先于 waitpid）也可能是信号退出，运行时按首次观测到死亡后 2 s 的宽限区分：宽限内 `exit = None`（UNKNOWN，退出码尚未收集，与 3.3+ 同一形态），之后才报 `Signal("unknown")`，否则正常 `exit 7` 会先闪一次 FAILED（agora-5nu；ubuntu-22.04 容器紧密轮询 30 次撞上 2 次，2026-09-03）；另有一条**与 3.2a 无关的**坑：进程经 PTY 交互输入之后退出时，tmux 有时根本不收集退出状态——pane 死了但 `pane_dead_status` 与 `pane_dead_signal` 都为空，而且永远不会补上。2026-09-03 查明根因（agora-tc4）：tmux **3.6 以前**链 libutempter 时，pane 死后 `server_destroy_pane()` 先关 fd（`pane_dead=1` 已经成立）再调 `utempter_remove_record()`，而 libutempter 在 fork 助手前把 SIGCHLD 置为 `SIG_DFL`（默认动作是忽略），恰好落在这几毫秒里的 pane 子进程 SIGCHLD 就被丢弃，tmux 再没人 `waitpid` 它，`PANE_STATUSREADY` 永不置位，子进程留成 defunct。上游 issue 4559，修复 fa5f3cef3「Send SIGCHLD after utempter_remove_record as well」进 3.6。macOS/brew 不链 libutempter，所以本机复现不出来。**补救**：tmux 的 SIGCHLD handler 是 `waitpid(WAIT_ANY, WNOHANG)` 循环，任何后来的 SIGCHLD 都会顺手收掉那个 zombie 并补上 status；所以运行时在 3.6 以下看到"已死却没有退出信息"时，用 `run-shell -b true` 让 server 自己 fork 一个短命子进程把 SIGCHLD 补上（每 socket 200 ms 一次、每个死会话最多 5 次；只对自己管的 socket 发，采纳来的用户 socket 仍然只读）。实测 ubuntu 24.04 / tmux 3.4（`read` 后 `exit 3` + send-keys + 密集轮询，30 次）：丢 7 次，每次 server 下恰好 1 个 defunct，补发后 7/7 拿回 `exit 3`。A12 的「退出码可读」因此对交互过的会话重新成立，`tests/invariants.rs` 的终局断言已收紧回硬断言，守卫见 `tests/tmux_dead_window.rs::dead_without_status_is_recovered_by_a_sigchld_nudge`。覆盖 Ubuntu 22.04（3.2a）、24.04（3.4，zuan 实机）、brew（3.7c）。daemon 启动时读 `tmux -V`（文档化接口）；低于下限或 server 协议不匹配（`brew upgrade tmux` 之后新 client 连不上旧 server）→ health 报 `runtime: degraded` 并给出原因，已有会话显示 UNKNOWN，**不杀 server、不退出 daemon**。这个结论是**实时**的（agora-xqa.4）：启动探一次版本，之后每次 list / inspect 的成败都更新它，成功即自动转回 ok，所以 server 换代后不必重启 daemon。降级期间读路径退化成"没有活性信息"（UNKNOWN + `runtime unavailable: <原因>`）而不是报错，但**写路径与 reconcile 仍然照常报错**——那两条路上把"读不到"当成"已经死了"会误写 `ended_at`、误判会话消失。守卫 `tests/runtime_degraded.rs`（假 tmux 二进制模拟 <3.2 与协议不匹配，含恢复）。安装脚本负责装 tmux；agora 的升级命令（A39）不碰 tmux。

**「server 不在」与「读不到」的分界（2026-09-21 修订，agora-5gg.17）**：上面那句「写路径与 reconcile 仍然照常报错」要和 D4 的「运行时会话没了 → FINISHED」放在一起读，否则会被读成「reconcile 报错的时候顺手把行判成没了」。分界只有一条：**运行时给没给出一个结论**，而不是「列表里有没有会话」。

- **没 server = 空列表，不报错**。`list_socket` 先 connect 那个 unix socket（结构事实，不匹配 stderr 文本，规则 10），连不上就直接返回空列表；`list-panes` 失败以后再探一次 socket，连不上也算「没有会话」——那是调用期间 server 正好退了，不是运行时坏了。reconcile 于是在 `live` 里找不到已知 ref，走 missing 那一格：补 `ended_at`（拿的是 daemon 此刻，标 `ended_at_approximate`）；**状态那一格不在 reconcile 里写**，由 `view()` 每 tick 给出 FINISHED `runtime_gone`（`server gone` / `session gone` 分开写 reason）——这是 D4 在 2026-09-19 据 agora-u5p 改判的那一条，运行时确实报不出退出时刻了，但 pane 随会话一起收 SIGHUP，agent 确定不在。
- **应答而拒绝 / 超时 / 报错 = 降级，不是空列表**。`ServerUnavailable` / `VersionMismatch` / `Timeout` / `Failed` 四种（判据 `RuntimeError::degrades_runtime()`）：读路径退化成 UNKNOWN + `runtime unavailable: <原因>`，`process` 报 `unknown` 而不是 `gone`；reconcile 这条路把 Err 直接抛给调用方（`src/main.rs` 记一句 warn 继续启动），一个 `ended_at` 都不写——那正是「把读不到当已死」会误写结束时刻、会误判会话消失的地方。
- **两条路在 `SessionManager::view()` 里共用同一道门**：`gone` 要求「本代已过 STARTING 窗口 + 没有降级 + 列表里没有它」三者同时成立，缺一条就落回 a23 / a17。**「降级」按 socket 算，不按整张列表算**（2026-09-22，agora-dkv3）：`list()` 对采纳 socket 的失败仍只 warn（用户自己的 tmux 坏了不得拖垮 agora 的会话列表、也不得把 `/api/health` 的 `runtime` 说成 degraded），但它随这次扫描交出 `RuntimeScan::unreadable`——那个 socket 上的行这一格读不到，其余 socket 上的行照常给结论。以前失败被整个吞掉，"列表里没有它"就只剩"会话没了"一种读法，adopted 行会被误判 FINISHED 并写 `ended_at`。`reconcile` 同一条门：落在没扫成的 socket 上的行进 `ReconcileReport::known_unreadable`，一个 `ended_at` 都不写。`runtime_status.observe()` 吃每一次 list / inspect 的成败，所以降级期间一恢复成功就转回 ok，不必重启 daemon（agora-xqa.4）。

守卫：`tests/runtime_degraded.rs`（假 tmux 二进制模拟 <3.2 与协议不匹配，含恢复；另两条钉采纳 socket 这次没扫成：`::unscanned_adopt_socket_keeps_adopted_rows_unknown` 读路径 + health 仍 ok、`::reconcile_skips_rows_on_an_unscanned_adopt_socket` 启动路径）、`tests/session_manager.rs::degraded_runtime_never_becomes_runtime_gone`（降级绝不变成 `runtime_gone`、绝不写 `ended_at`）、`::starting_window_exempts_a_row_that_is_still_starting`、`::reconcile_covers_all_six_cases`、`tests/status_truth_table.rs::a17_an_unreadable_runtime_is_unknown_not_gone` / `::a19_a_missing_runtime_session_is_not_unknown` / `::a23_a_row_the_runtime_has_not_reported_yet_is_not_gone`（三格各钉一个落点）。口径汇总在 `docs/spec/status.md`。
