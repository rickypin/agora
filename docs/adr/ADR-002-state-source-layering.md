...
- **进程层会说的结论不止「退出码」这一种（2026-09-21 修订，agora-5gg.17）**：有 `runtime_ref`、运行时正常应答、而列表里没有这个会话 ⇒ 进程层给出 **FINISHED**（`source = process`、conf 0.8、`end_cause = runtime_gone{session|server}`），**不是** UNKNOWN `runtime session missing`；运行时整体读不到（降级）⇒ 仍是 UNKNOWN `runtime_unavailable`、`process = unknown`，且绝不写 `ended_at`。这两格长得不一样，而它们共用一道门：`gone` 要求「没有降级 + 本代已过 STARTING 窗口 + 列表里没有它」同时成立。**规范出处在 ADR-001**，不在本节：D4 的「daemon 重启」与「运行中 server 没了」两行（2026-09-19 据 agora-u5p 改判）与 D7 的「server 不在（空列表）≠ 读失败（降级）」分界（2026-09-21 补），conf 只给 0.8 的两个已知假阳性也在那里。本节只登记它对裁决规则的影响：上面那条「进程退出压倒一切」里的「退出」现在包含「会话连同 pane 一起没了」（pane 进程收 SIGHUP，agent 确定不在是事实），但**永远不包含「读不到」**。守卫 `tests/session_manager.rs::degraded_runtime_never_becomes_runtime_gone`、`::missing_runtime_session_finishes_the_row_and_writes_ended_at_once`、`tests/status_truth_table.rs::a13_finished_when_the_runtime_session_is_gone` / `::a17_an_unreadable_runtime_is_unknown_not_gone` / `::a19_a_missing_runtime_session_is_not_unknown` / `::a23_a_row_the_runtime_has_not_reported_yet_is_not_gone`；四列的合法格与不可能格在 `docs/spec/status.md`。
- **有 hook 的 agent**：文本层永远不能把状态抬到 WAITING / TURN_DONE；活动层不能产生 IDLE。它们只提供 pane preview。
- **hook 沉默规则**（有 hook 的 agent 的兜底）：SessionManager 在达到沉默阈值后才 capture 屏幕并交给 Adapter；hook 健康时不 capture，有 hook 的会话不展示 pane preview。文本结果只能导致 UNKNOWN，绝不能抬到 WAITING / TURN_DONE（agora-uez：守卫须从 SessionManager 入口覆盖，单测直接喂 Machine 文本会漏掉接线错误）。进程活着、`hook_silence_after`（默认 10 min）内没有任何 hook 事件、且文本层看见了空闲提示符或权限提示 → **UNKNOWN**，reason `hooks silent`。这是"宁可 UNKNOWN"的落点：hook 没装好、被 `disableAllHooks` 关掉、或 agent 升级改了事件名时，Dashboard 显示"看不清"而不是永远 RUNNING，也不会误报 WAITING。补充（2026-09-05，agora-dvh.15）：沉默规则要等 10 min 且屏幕像在等人，对"hook 从来没接上"太迟钝——Codex 未在 `/hooks` 信任、`disableAllHooks`、二进制路径失效都表现为一条事件都没有。所以另有一条更早的提示：声明了 hook 的会话在本代起始 10 s 宽限之后有过真实终端输出（不算 TUI 启动那波，因为 Codex 的 SessionStart 到第一条 prompt 才 fire）、再过 `hooks.unheard_after`（默认 90 s）仍无任何事件 → `SessionView.hooks_unheard` 带一句指向 `/hooks` 或重装的话（不改状态，状态仍按进程层）；第一条事件到达即撤。守卫 `tests/state_machine.rs::hooks_never_heard_after_terminal_activity_is_flagged_but_not_at_startup`。例外（2026-09-06，agora-9cd）：有挂起的权限 / 提问时每 tick 都 capture、不等沉默阈值——只用来判断提示是否还在屏幕上（D5 的解除途径"屏幕上的权限提示消失"）；文本层在这里同样只能把 WAITING 降成 UNKNOWN、不能抬，hooked 会话的 `preview` 仍为 null。守卫 `tests/api_input.rs::an_interrupted_permission_is_released_via_terminal_when_the_prompt_disappears`。
- **两条沉默规则从哪个时刻算（2026-09-19 修订，agora-5gg.2）**：上面的沉默规则与 STARTING 的 `startup_grace` 衰减按 **daemon 收到 hook 的时刻**算——它们问的是"daemon 从上次听到 hook 起还没等到下一条多久"，而屏幕证据要 daemon 活着才采得到，所以停机三天后重放一条三分钟前的 SessionStart 不能让它当场衰减。但无句柄 external 行的 `hooks.external_silent_after` **改按最近一条 hook 事件自己的时刻**（投递件落盘那一刻）算：那条回答的是"agent 大概率早退了"，事件三小时前发生就是三小时前没声音，daemon 停机与重放不该把"沉默了多久"清零。2026-09-18 盘点（`docs/analysis/session-status-audit-2026-09-18.md` §4 A4，判 (b)）的现场：Mac 停机 3.5 天，被重放的那批 Codex Desktop 行（0e26ad / 14d791 / eb129d / b939bb）把时钟拨到重放时刻、沉默 62 h 仍 TURN_DONE，同一批里没被重放、从检查点恢复的 4 行却当场 UNKNOWN——同一个规则两种结果。事件时刻随 hook 检查点落盘，重启不清零；读不出事件时刻的旧检查点退回按收到时刻算（= 修订前的行为，不会因为缺键把还在等的行提前打成 UNKNOWN）。UNKNOWN 自己的起点仍是判定的那一个 tick，不是事件时刻（前端拿它算 "unknown 3m"）。守卫 `tests/state_machine.rs::handleless_silence_is_measured_from_event_time`、`tests/hook_recovery.rs::handleless_external_silence_is_not_cleared_by_replay_or_restart`。
...
- **每个状态值带 `source`、`confidence`、`reason`**；UI 不显示 confidence，但 API 返回，日志记录。`source` 的取值是 `hook | process | text | activity | none`（2026-09-21 按代码回写，agora-5gg.17：第四层叫活动启发式，`Source` 取的是 Activity 这个名字而不是 `heuristic`；`none` 只在「这一层说不出结论」时出现，因而只能与 UNKNOWN 同现——a17 / a23 / x12 三格）。
- **"为什么"是一个枚举，不是一句话（2026-09-20，agora-5gg.6）**：`reason` 从此只是给人看的那一句，措辞可以随版本改；程序按 `end_cause`（`exit_code | signal | killed_by_user | host_session_end{clear|resume|logout|exit|other} | superseded | process_gone | runtime_gone{session|server}`）与 `unknown_cause`（`runtime_unavailable | hooks_silent_screen | prompt_gone | hooks_silent_no_handle | no_observation | exit_status_missing`）分支。动因是本 ADR 上面那几条的副作用：`SessionEnded(reason)` 一律映射成 `session ended (hook)`、宿主的 reason 被吞掉，而探活结束又叫 `external process gone`、运行时会话没了又叫 `runtime session gone (…)`——于是 §2.3 规则 10（程序不得靠给人看的文本做判断）在 `src/events.rs` 那里破了一个洞：通知静音写的是 `reason.starts_with("killed by user")`，external 行分"人自己结束的"与"崩溃的"靠的是 `source != process`。现在两条都按枚举判（守卫 `src/events.rs` 单测 `notification_for_branches_on_end_cause_not_on_the_reason_text`，`Transition` 里根本没有 `reason` 这一格）。宿主原话不进枚举（词表随各家版本变，认不出的一律 `other`），只留在 `reason` 里。守卫 `tests/status_truth_table.rs::every_finished_and_failed_row_names_its_end_cause`、`::every_unknown_row_names_why_it_is_unknown`、`::cause_wire_vocabulary_is_the_locked_set`；形态见 `docs/spec/api.md`「会话形态」，随 `api_version` 1.8 只增字段。
- **起好了、还没收到第一条指令（2026-09-05，agora-okr）**：SessionStart 映射为 hook 层 STARTING（conf 1.0），而 Claude 在用户提交第一条 prompt 之前不再发任何事件，原样保留会永远钉在 STARTING（唯一出口是 10 min 沉默 → UNKNOWN）。所以 hook 层的 STARTING 带衰减：SessionStart 之后 `startup_grace`（10 s，与"hook 没接上"共用同一宽限）内没有后续 hook 事件、进程活着 → TURN_DONE（source=hook，conf 0.9，reason `session started, awaiting first prompt`）；PromptSubmitted / Activity 照常回 RUNNING，STARTING → TURN_DONE 这一步不发浏览器通知（只有 RUNNING / IDLE 出发的转换才通知）。不能改成 SessionStart 直接映射 TURN_DONE：`source=compact` 发生在一轮中间，紧接着就有 PreToolUse 在宽限内自纠。沉默规则优先级更高：10 min 无事件且屏幕像在等人仍是 UNKNOWN。daemon 重启从检查点恢复的 STARTING 在之后的 tick 同样衰减（检查点只在 hook 事件时写，衰减不写盘）。守卫 `tests/state_machine.rs::hook_starting_decays_to_turn_done_awaiting_first_prompt`、`::restored_hook_starting_decays_too`、`tests/hook_recovery.rs::starting_decays_through_session_manager_and_after_checkpoint_restore`。补充（2026-09-19 修订，agora-rkl）：这条衰减**不分 origin、也不分有没有可信进程号**——它回答的是「这一行要我做什么」= 给它指令，与谁起的无关。规则原先只写在 `observe_hooked` 里，而无句柄 external / headless 行的进程层恒为 `Source::None`、`Machine::observe` 第 1 步就 return，对它们根本不可达；现场 zuan 的 `ef0e50` 只收到过一条 SessionStart，于是钉在 `… starting` 180 h（`docs/analysis/session-status-audit-2026-09-18.md` §3.1 / A1）。修法不是拆掉第 1 步的 return（那会让这些行落到「进程层 RUNNING」这种它们根本没有的事实上），而是在第 1 步的 external 分支里落同一个 `Machine::decay_starting`：`Liveness::Alive | Unknown` 都衰减，`Dead` 不衰减——进程事实压倒 hook。守卫 `tests/status_truth_table.rs::a03_hook_starting_never_outlives_the_startup_grace`、`::x02_handleless_starting_never_outlives_the_startup_grace`（两格共用同一条代码）、`tests/state_machine.rs::handleless_external_starting_decays_to_turn_done`、`tests/hook_recovery.rs::handleless_external_starting_decays_through_session_manager_and_after_checkpoint_restore`。
...
| session.ended → FINISHED（hook 层，conf 0.8，reason `session ended (hook)`）；进程退出后由进程层以 1.0 定 FINISHED / FAILED 覆盖；`reason = clear` 不改状态（进程活着、紧接着新 id 的 SessionStart 落回同一行）；无句柄的 external 行新 id 会另起一行，receiver 把它的 `clear` 改写成普通结束（agora-s3r，2026-09-07）。2026-09-06 agora-vfi 起——之前是"只清挂起、等进程退出"，对没有进程事实的 external 会话（Codex Desktop 线程的 ppid 是共用 app-server）就永远 UNKNOWN | `SessionEnd`（`reason`: clear / resume / logout / prompt_input_exit / other） | `SessionEnd`（`reason = shutdown`） | `SessionEnd`（`reason = other`；/clear 不发） | 进程退出 |
...
daemon 侧：启动时先恢复 `hooks/state/` 的每会话 hook 观测检查点，再按文件名顺序重放 inbox（§3.4）。**重放不是一次性的**（2026-09-21 修订，agora-5gg.17；实施 2026-09-19 agora-5gg.1）：那一次扫描只看得到它跑那一刻的快照，而 unix socket 在那之前就 bind 了，bind 与 accept 之间到达的 hook 只能指望内核 backlog——任何一个没排上队的（backlog 满、connect 被拒、客户端被宿主按 timeout 杀掉）投递件就再没人读，一直等到下次重启（§5.1「不得丢失」不等价于「daemon 忙着的时候也不得丢失」；2026-09-18 Mac：replay 跑了 172 s，窗口里落下的 6 个 Pre/PostToolUse 一直躺在投递箱）。所以 serve 起来之后要立刻再扫一次，之后 sweep 每周期兜底一次（`Receiver::replay_stale_pending`）。两条保证幂等：应用成功就把文件挪进 `done/`，挪之前崩了也有检查点里的「已处理文件名」挡同名重放；只碰落盘早于 `hook::PENDING_REPLAY_AGE`（30 s）的投递件——刚落盘的那条多半正有人在 socket 上等答复，抢下来会让那条连接拿到「路径不在投递箱里」的错误、这次权限请求也进不了挂起表。守卫 `tests/hooks_inbox.rs::events_landing_during_replay_are_consumed_without_restart`、`::freshly_written_deliveries_wait_out_the_grace_period`、`::stale_replay_checks_the_hooks_root_too`（启动那道权限门要连 `hooks/` 根一起查，否则它等于只在启动那一刻有效）。检查点格式带 `version`（1 → 2，2026-09-08 agora-tql；2 → 3，2026-09-09 agora-2nh），只含 hook 状态、摘要、待回答工具键、epoch、原观测时间、最后已处理文件名、最近一条 hook 事件自己的时刻（`last_event_at`，agora-5gg.2 起；纯增一个可缺省的键，不升版本号——旧检查点读不出它就退回按收到时刻算，见 D1），以及 v2 起最近一条信封里的 agent 进程号、其启动时刻与报来它的那条 hook 的时刻（`agent_process`；`seen_at` v2 是秒、v3 起是毫秒——同一进程先后两个对话谁新谁旧靠它分，两条 hook 常在同一秒内，秒级会让平局落到内存表的迭代顺序；加载 v2 时换算）——那是"最后看到的进程"这个 hook 事实，不是存活结论；不含进程存活、退出码或挂起连接，也不进入 SQLite。每次应用 hook 后以 0600 临时文件写入、sync、原子替换检查点，成功后才将投递移到 `done/`（保留 24 h 排障）——挪进去时截断大结果字段：`tool_response` / `toolResult` 超过 8 KB 的部分换成 `…[truncated N bytes]`，`tool_input` 不动（agora-pzi 的权限摘要要从它取主参数，归档重建也靠它；这两个被截的键没有任何 adapter 的 parse 读，所以重建结论不受影响）。清理按 `PRUNE_INTERVAL`（1 h）节流挂在 sweep 周期上，而不是只在启动重放的尾部跑一次——后者让"保留 24 h"实际变成"下次重启时清掉 24 h 以前的"，daemon 不重启就无界增长（2026-09-10 现场：开发机 `done/` 87 MB / 12274 文件，64.4 MB 里 30.4 MB 是那两个结果字段，grok 还把同一份结果按两种键名各存一遍；agora-t36）；重复或更旧的同 epoch 文件不再次应用，旧 epoch 检查点不恢复。运行时退出 / 缺失的事实始终覆盖恢复状态。首次升级尚无检查点的会话从现存 `done/` 按序补建；无句柄 external 行的 v1 检查点在 `done/` 里还有投递时也丢掉检查点从归档重建（v1 可能停在 s3r 修复前的结论，agora-tql）；归档已删除的历史无法凭空恢复，下一条 hook 会建立检查点。检查点不受 24 h 归档清理影响；但 external 行结束超过 `sessions.external_finished_ttl`（默认 24 h）被 daemon 自动删 metadata 时，检查点随之删掉（与 Delete metadata 同一条路径，`SessionManager::delete_metadata`）——`restore_hook_checkpoints` 只按 metadata 行找检查点，不删就成了 `hooks/state/` 里永远没人读的孤儿文件（agora-j4w.3，守卫 `tests/external_expiry.rs::expiry_takes_the_delete_metadata_path_and_drops_the_hook_checkpoint`）。external 行的存活原来只靠内存里的进程号，daemon 一重启就丢、agent 早退了的行永远钉在 TURN_DONE（2026-09-08 现场 11 行）：进程号进检查点、重启回填，启动时刻对不上视为号被复用；没有可信进程号的行 hook 沉默超过 `hooks.external_silent_after` 退到 UNKNOWN（agora-tql；沉默时长从最近一条 hook 事件自己的时刻算，agora-5gg.2，见 D1）。2026-09-05 review 反例（agora-9dj）：只重放 inbox 会让已消费 Stop 的 TURN_DONE 在重启后永远退回 RUNNING。**不再有 `POST /api/hooks/:agent`**：unix socket 仅属主可访问 + 对端 uid 校验（ADR-003 D6）就是"这台机器上的这个用户"，多用户主机上比 loopback 端口 + 一次性 token 更准确（§0.1、ADR-003 少一个要守的端点）。Windows 时代换 named pipe，同属第二运行时（ADR-001 D9）。
...
- **与终端并存**：实测 Claude Code 在 hook 挂起期间照常显示权限提示——终端里答了，TUI 走自己的路；Dashboard 答了，hook 返回决定。所以不需要"有人 attach 就放弃挂起"的逻辑。挂起在下列任一发生时自动解除（`decision.resolved`）：同 `tool_use_id` 的 PostToolUse / PostToolUseFailure / PermissionDenied 到达（终端答了）、Stop / SessionEnd 到达、进程退出、超时、屏幕上的权限提示消失 ≥ `text_ticks` 个 tick（文本层见过提示又见它没了；状态降为 UNKNOWN 而不是猜 RUNNING，因为 Esc 中断与工具正在跑在屏幕上分不出来；2026-09-05 Claude 2.1.261 实录 Esc 一个事件都不发，agora-9cd）。最后一条的细则：从没在屏幕上见过提示就不动（检测器认不出这家的权限 UI 时宁可维持 WAITING，退回沉默兜底）；不做"挂起过期"，用户离开几小时再回来答权限是正常的；同一秒多次读只算一个 tick；状态机清了挂起之后 receiver 的 `sweep`（每 5 s）把状态机里已无此键、登记 ≥ 2 s、同一 epoch 的 hold 以 none 放掉（hook fail-open 退出，`pending_decision` 移除，`decision.resolved` via=terminal）；之后 PostToolUse 的解除事件到了把 UNKNOWN 抬回 RUNNING；这个 UNKNOWN 钉住直到下一条 hook 事件写出新状态（含 Notification(idle_prompt) → TURN_DONE(hook, reason `idle`)，2026-09-06 agora-01g：Esc 之后 Claude 停在提示符约 60 s 就发 idle_prompt，那正是"一轮做完、等下一条指令"的可信证据，不接它行会 UNKNOWN 到用户的下一条 prompt；沉默规则的 UNKNOWN 同样——谓词是 `Unknown ∧ source=text`，两条规则共用；挂起还在的 WAITING 里 idle_prompt 仍不动）。守卫 `tests/state_machine_pending.rs::pending_permission_is_released_when_the_prompt_leaves_the_screen`、`::pending_permission_stays_while_the_prompt_is_on_screen`、`::pending_permission_never_seen_on_screen_is_left_alone`、`::dashboard_or_terminal_resolution_still_wins`、`::idle_notification_after_the_prompt_is_gone_lands_on_turn_done`、`::idle_notification_while_the_prompt_is_still_pending_is_ignored`、`tests/state_machine.rs::idle_notification_after_silent_hooks_unknown_lands_on_turn_done`、`tests/api_input.rs::an_interrupted_permission_is_released_via_terminal_when_the_prompt_disappears`。
...
**修订（2026-09-21，agora-5gg.17）**——本节的身份规则不管「该不该登记」，而那件事在 D3 的接收侧；读者要找「哪种会话该有一行」得从这里跳，所以两条都记在本节。

- **身份交接不留行（2026-09-20 agora-29n）**：无句柄的 external / headless 行在状态机还没见过第一条 `prompt.submitted` 时遇到 `SessionEnd(reason = resume | clear)`，说的是**身份交接**而不是这一行的对话结束了——走与 `DELETE /api/sessions/:id` 同一条路径删（hook 检查点一起删，`done/` 里的归档也不重建它）。动因就是本节这条 D7：身份 = `(host, agent_session_id)`，于是 `claude --resume` 先以**新 id** 发 `SessionStart(source=startup)` 就登记出一行，12 s 后的 `SessionEnd(reason=resume)` 落到它身上，留下一行没有 prompt、没有输出的 FINISHED 挂在侧栏（2026-09-18 zuan 现场 `e3cd17`，`docs/analysis/session-status-audit-2026-09-18.md` §3.4）。判据是状态机没见过人话（`SessionManager::hook_prompt_seen`；宿主注入的 `prompt.injected` 不算，agora-3s5）。两侧都不越界：有 `runtime_ref` 的行完全不碰（它们的身份是 pane，SessionEnd 之后同一进程再发的 SessionStart 落回同一行，那是真 resume），有过 prompt 的 external 行按普通结束（agora-s3r）。守卫 `tests/hooks_external.rs::a_resume_handoff_before_the_first_prompt_leaves_no_row`；形状见 `docs/spec/architecture.md`「身份交接删行」。
- **无句柄这一档再分两种 `origin`（裁决 agora-5gg.7 选 B；实施 2026-09-20 agora-5gg.20）**：`external`（人在终端里自己起的）与 `headless`（宿主自己起的一次性会话与子代理）。**身份规则一个字没改**：两种都按 `(host, agent_session_id)` 认行、都靠 supersede 换行、都没有 pane 与 Restart，代码里一律问 `Origin::is_handleless()`。判据只用登记它那一条投递件的 `SessionStart` 载荷结构（缺交互模式才带的四个键），不看环境变量、不看进程树、不猜 prompt 文本（§2.3 规则 10）；录不出结构差异的宿主（`codex exec` 的 SessionStart 与交互 TUI 逐键相同）不猜、照常按 `external` 登记。与 D7 有关的只有一件事：supersede 只发在这两种来源上，所以「一个进程挂多行」只在无句柄那一档发生（有句柄的行身份是 pane，同一进程再发 SessionStart 落回同一行）。三处不同（不通知、侧栏不看状态即折叠、到期不论状态即删）与守卫见 `docs/spec/status.md` §5 与 `docs/spec/api.md`「external 与 headless」。
