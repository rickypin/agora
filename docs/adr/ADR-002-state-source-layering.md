...
daemon 侧：启动时先恢复 `hooks/state/` 的每会话 hook 观测检查点，再按文件名顺序重放 inbox（§3.4）。检查点格式带 `version`（1 → 2，2026-09-08 agora-tql），只含 hook 状态、摘要、待回答工具键、epoch、原观测时间、最后已处理文件名，以及 v2 起最近一条信封里的 agent 进程号与其启动时刻（`agent_process`）——那是"最后看到的进程"这个 hook 事实，不是存活结论；不含进程存活、退出码或挂起连接，也不进入 SQLite。每次应用 hook 后以 0600 临时文件写入、sync、原子替换检查点，成功后才将投递移到 `done/`（保留 24 h 排障）；重复或更旧的同 epoch 文件不再次应用，旧 epoch 检查点不恢复。运行时退出 / 缺失的事实始终覆盖恢复状态。首次升级尚无检查点的会话从现存 `done/` 按序补建；无句柄 external 行的 v1 检查点在 `done/` 里还有投递时也丢掉检查点从归档重建（v1 可能停在 s3r 修复前的结论，agora-tql）；归档已删除的历史无法凭空恢复，下一条 hook 会建立检查点。检查点不受 24 h 归档清理影响。external 行的存活原来只靠内存里的进程号，daemon 一重启就丢、agent 早退了的行永远钉在 TURN_DONE（2026-09-08 现场 11 行）：进程号进检查点、重启回填，启动时刻对不上视为号被复用；没有可信进程号的行 hook 沉默超过 `hooks.external_silent_after` 退到 UNKNOWN（agora-tql）。2026-09-05 review 反例（agora-9dj）：只重放 inbox 会让已消费 Stop 的 TURN_DONE 在重启后永远退回 RUNNING。**不再有 `POST /api/hooks/:agent`**：unix socket 仅属主可访问 + 对端 uid 校验（ADR-003 D6）就是"这台机器上的这个用户"，多用户主机上比 loopback 端口 + 一次性 token 更准确（§0.1、ADR-003 少一个要守的端点）。Windows 时代换 named pipe，同属第二运行时（ADR-001 D9）。
