...
| session.ended → FINISHED（hook 层，conf 0.8，reason `session ended (hook)`）；进程退出后由进程层以 1.0 定 FINISHED / FAILED 覆盖；`reason = clear` 不改状态（进程活着、紧接着新 id 的 SessionStart）。2026-09-06 agora-vfi 起——之前是"只清挂起、等进程退出"，对没有进程事实的 external 会话（Codex Desktop 线程的 ppid 是共用 app-server）就永远 UNKNOWN | `SessionEnd`（`reason`: clear / resume / logout / prompt_input_exit / other） | `SessionEnd`（`reason = shutdown`） | `SessionEnd`（`reason = other`；/clear 不发） | 进程退出 |
