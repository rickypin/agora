...
事件流 Server → Client 每帧一个 JSON **数组**（服务端 ~50 ms 合并突发，同一会话的连续状态变化只留最后一条）：`{ "type": "session_created", "id", "session" }`、`{ "type": "session_removed", "id" }`、`{ "type": "session_updated", "id", "session" }`（metadata 或 pending_decision 改了；整行重发）、`{ "type": "status_changed", "id", "status", "source", "reason", "process", "alive", "detail", "prompt", "progress", "preview", "status_since", "hooks_unheard" }`（`detail` / 两行预览 / pane 预览 / `hooks_unheard` 变了也算状态变化；`process` 变了同样算——status / source / reason 一个字没变而进程从 `alive` 换成 `unknown` 是要人看一眼的变化，agora-5gg.18；`process` 与 `alive` 同时带，没升级的节点不发 `process`，页面就退回 `alive` 投影读（`rowProcess()`），不会留下一个与 `alive` 打架的旧三值；`task` 的到达走 `session_updated`）、`{ "type": "decision_resolved", "id", "tool_use_id", "via": "dashboard" | "terminal" | "session" | "exit" | "timeout" }`（挂起的决定被解除，ADR-002 D5；`terminal` = 同工具的 PostToolUse / PostToolUseFailure / PermissionDenied 到达——终端里答了——或屏幕上的权限提示消失 ≥ `text_ticks` 个 tick 后状态机已无此挂起、receiver 的 sweep 把还在等的 hook 放掉，agora-9cd）、`{ "type": "notification", "id", "title", "body", "status" }`（浏览器通知，MISSION §6.6 / A18：只在 RUNNING（或 IDLE）→ WAITING / TURN_DONE / FINISHED / FAILED 四种转换上各发一条，紧跟该会话的 `status_changed` 或 `session_updated` 之后；`title` 是 §6.6 表的文案 `<Agent> / <name> @ <node> needs input | finished its turn | finished | failed`，`body` 是 `detail` 首行，`status` 是转换后的状态，前端据此决定点击落到哪——WAITING / TURN_DONE 落到该行并把焦点放进主区回答面板（A50）；`notifications.enabled = false` 时一条不发；RUNNING → IDLE / UNKNOWN、STARTING → FAILED、TURN_DONE → FINISHED、用户自己 Kill 的不通知；`origin = external` 的行落到 FINISHED 时只有 `source = process`（进程消失：崩溃、被杀、Codex 关窗口——它不发 SessionEnd，bd memories `external-exit-hooks-ctrlc-vs-hup`，接受被当成意外通知一次）才通知，`source = hook`（SessionEnd、`/clear` 换对话的 superseded）是人在终端里自己结束的、同 Kill 一样不通知（agora-j4w.4；MISSION §4.6 证据 ②；守卫 `src/events.rs` 单测 `external_rows_ended_by_hook_do_not_notify_but_process_gone_does`）。这条 external 的分档只在转换前是 RUNNING / IDLE 时才轮得到：回完一句停在 TURN_DONE 的行怎么结束都不通知——上一句的 `TURN_DONE → FINISHED` 先把它挡住了，`source` 是 hook 还是 process 都一样，所以要验这条分档得在 agent 正忙（RUNNING）时结束会话（2026-09-09 真宿主实测，agora-agu））、`{ "type": "peer_changed", "name", "peer": { "online", "last_seen", "retrying", "last_error" } }`（agora-c8h：一个 peer 在本节点眼里的面貌变了，`peer` 就是「Health」peers 段里该节点的那一项；只在 `online` / `retrying` / `last_error` 任一变时发，在线期间 `last_seen` 的刷新不发；同一批里同一 peer 只留最后一条；断线时它在该 peer 各行的 stale `session_updated` 之前、恢复时在全量并入的差分事件之后。Header 的点据此与侧栏行同一眼变，不等 health 轮询；守卫 `tests/peer_stale.rs::offline_peer_is_stale_with_last_seen_not_removed` / `recovery_resyncs_full_snapshot`、`src/peer/state.rs` 单测 `face_changes_are_published_and_last_seen_refreshes_are_not`）、`{ "type": "resync" }`（服务端丢过该客户端的事件，必须重拉全量）。Client → Server 只有 `{ "type": "ping" }` → `{ "type": "pong" }`。进程状态的变化由 daemon 按 `status.detector_interval` 轮询 Session Manager 求差发出；API 自己做的增删立即发。两个 WS 升级都先过 principal，再校验 `Origin` 与 `Host` 同源（403 `cross_origin`）。升级之后凭据每 5 s 复查一次：吊销 / 轮换 → 服务端以关闭码 `4401 revoked` 关流（「认证」末条）。

运行时会话已经不在的行（`status = finished` 且 `reason` 含 `runtime session gone`，见「REST」的会话形态）**不该建这条 WS**：那一行没有 pane，attach 只有一次 `no sessions` + 退出码 1。前端（`web/src/terminal.ts::runtimeSessionGone` + `web/src/TerminalView.tsx`）在那一格画「运行时会话已不在」与两个真出口（Restart 走 `POST /api/sessions/:id/restart`，节点侧退化成同名 `create`；删除记录走 `DELETE /api/sessions/:id`），不画「重新连接」。服务端不拦这个调用（`GET`/`WS` 的对端还有 CLI 与 peer），改的是这一格给用户的出口。
...
- **没有 HTTP 端点**。agent 的 hook 命令是 `<AGORA_HOME>/bin/agora hook --host <agent> --home <AGORA_HOME>`（安装写入的完整形态见 ADR-002 D4）：payload 落到 `<AGORA_HOME>/hooks/inbox/<host>/<agent_session_id>/<ts>-<seq>.json`（先 `.part` 再 rename），再经 `<AGORA_HOME>/agora.sock`（unix socket，仅属主可访问 + 对端 uid 校验，ADR-003 D6）唤醒 daemon；daemon 不在时文件留着，启动时按文件名顺序重放（MISSION §3.4）；应用后先将 hook 观测原子写入 `<AGORA_HOME>/hooks/state/` 的版本化每会话检查点（0700 目录、0600 文件），成功才移到 `<AGORA_HOME>/hooks/done/` 保留 24 h 排障——挪进去时把 `tool_response` / `toolResult` 截到 8 KB 并留 `…[truncated N bytes]` 标记（归档只为排障；状态机此刻已消费过完整 payload，`tool_input` 不截，权限摘要还要用它），清理挂在 daemon 的 sweep 周期上、按小时节流，不必等下次启动（agora-t36）；启动先恢复检查点再消费更新的 inbox，旧版无检查点时从保留的 done 补建；无句柄 external 行的 v1 检查点（2026-09-08 之前写的，可能停在 agora-s3r 修复前的结论）在 done 里还有它的投递时丢掉检查点、从 done 按序重建（agora-tql）。检查点（`version = 2`）含 epoch 与已处理文件名以去重、最近一条信封里的 agent 进程号与其启动时刻（`agent_process`，external 行探活用；这是"最后看到的进程"这个 hook 事实，活没活仍每 tick 现算），不含 alive / exit 或挂起连接，不影响运行时作为存活真相源；恢复 WAITING 后 pending_decision 为 null，不能再向已断开的 hook 发送批准；信封里 `AGORA_EPOCH` 小于会话当前 epoch 的事件丢弃（Restart 之前那代进程发的）；`hooks/` 下任何一级目录属主不对或 group / other 有位，daemon 拒绝读并记日志（agent 照跑），兜底重放这条路同样先查 `hooks/` 根目录本身加 `inbox/` 那一支（不陪着扫 `done/`）、过宽就整轮跳过——`hooks/` 过宽时启动拒读、5 s 后兜底放行，这道门就等于只在启动那一刻有效（守卫 `tests/hooks_inbox.rs::stale_replay_checks_the_hooks_root_too`）。启动那次重放只扫它那一刻的快照，而 unix socket 在那之前的 reconcile / 重放跑完之前就 bind 了：bind 与 accept 之间到达的 hook 排内核 backlog（std UnixListener 128），没排上队的投递件过去再没人读、一直躺到下次重启（2026-09-18 Mac：重放跑了 172 s，窗口里落下的 6 个 Pre/PostToolUse 至今没进 done）。现在由 socket 起来之后的补扫与每 5 s 一轮的 sweep 兜底重放消费，只碰落盘早于 30 s 的投递件（`hook::PENDING_REPLAY_AGE`）——刚落盘的那条多半正有人在 socket 上等答复，抢下来会让那条连接拿到"路径不在投递箱里"的错误、也让这次权限请求进不了挂起表（agora-5gg.1；守卫 `tests/hooks_inbox.rs::events_landing_during_replay_are_consumed_without_restart`、`tests/hooks_inbox.rs::freshly_written_deliveries_wait_out_the_grace_period`）。
...
- **等待时长是个下界，peer 行的下界画成下界**（agora-5gg.12；ADR-004 附录）：`GET /api/sessions` 的响应顶层带一个 `now`——打这份快照时报告方自己时钟的读数（unix 秒；Human 与 Peer 两种 principal 都带）。并入方只拿它减行里的 `status_since`：同一只表上的两个读数之差是相对量，两节点之间的时钟偏差在减法里自己抵消，所以"不信 peer 报的时间"守得住，而本机 daemon 重启（`PeerViews` 纯内存、重启即空）不再把 peer 上报 8 天的 STARTING 画成刚刚起的（2026-09-18 Mac 实测：重启后 zuan 的 24 行全是 0.7 h）。两条限制：只在本节点**第一次**看见该状态时用它把起点往前推（含重启后的第一眼），状态没变就不动那个起点——`status_since` 后退会改掉前端「看过」的键 `<id>@<status_since>`（agora-23h），把已经看过的 FINISHED 行弹回 NEEDS ATTENTION；差读不出 / 为负（peer 的时钟在一次驻留里倒跳）/ 超过 30 天（`status_since` 坏成 0）一律当它没报，退回本机第一次看见它的时刻——坏掉的那个差不是下界，截断到 30 天照样是撒谎，退回本机此刻只会短、不会长。老节点不报 `now`、以及事件流带来的新状态（`status_changed` 里没有 `now`）都退回本机那一刻。因为整个数就是"本机至少看了它多久"，peer 行的时长在 UI 上带 `≥`（`web/src/attention.ts` 的 `statusLine`，判据是行上有 `stale` 键）；本机行的起点是自己打的，精确、不带 `≥`。守卫 `tests/peer_view.rs::a_local_restart_does_not_reset_a_peer_rows_wait`、`src/peer/view.rs` 单测 `a_local_restart_keeps_the_wait_the_peer_reports`、`::a_later_snapshot_does_not_push_an_existing_stamp_earlier`、`::an_untrusted_reported_wait_is_ignored_rather_than_guessed`、`web/src/attention.test.ts`（peer 行时长文案）。
