...
| 环节 | 规则 | 为什么 |
|---|---|---|
| 判"没了" | `SessionManager::view` 里的 `gone`：`degraded` 为空 **且** `live` 里没有这个 ref。降级（协议不匹配、超时、server 应答不了）永远不算，那一臂照旧 UNKNOWN `runtime unavailable: …`（ADR-001 D7） | "读不到"与"运行时答话说没有"是两件事；拿前者当后者会误写 `ended_at`、误报结束 |
