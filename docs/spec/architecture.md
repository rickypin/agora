...
│  ┌─────────────────────────────────────────────────────────┐  │
│  │ Dashboard │  Respond   │ xterm.js │ Notifications       │  │
...
| 登记 | 库里没有时，**先看事件**：`parse` 出来为空、或只有 `SessionEnded` 的**不登记**（debug 日志"没见过的会话的 SessionEnd，不登记"），返回 None；其余按信封里的运行时环境定位 pane → 定位到可采纳 socket 的以 `adopted` 登记（有终端），否则 `external`（无句柄）。agent 没自报 id（`unknown`）的照旧不登记。三家宿主同一规则。**无句柄这一档再按登记它那一条投递件的载荷结构分两种 origin**：`SessionStart` 缺交互模式才带的四个键（`src/adapter/claude.rs` 的 `INTERACTION_ONLY_KEYS` = `model` / `scratchpad_dir` / `permission_mode` / `effort`）→ `headless`（宿主自己起的 `claude -p` 一轮、宿主内部的子代理；裁决 agora-5gg.7 选 B，实施 agora-5gg.20），其余仍是 `external`。判据只在 `SessionStart` 上问、只在登记那一次问（错过它的行不回头补判）；录不出结构差异的宿主（`codex exec` 的 SessionStart 与交互 TUI 逐键相同）不猜、照常按 `external` 登记。两者共用同一套无句柄语义（探活 / supersede / 归档重建 / `ended_at`，判据 `Origin::is_handleless()`），区别三处：不通知（`notification_for`）、侧栏一律折叠（`web/src/attention.ts`）、到期不论状态即删（`expire_external_finished`）——口径见 `docs/spec/api.md`「外部会话」，守卫 `tests/hooks_external.rs::a_headless_session_registers_as_headless_and_expires_whatever_its_status_is`。`external` 这一档再按**登记它那一条投递件的载荷结构**分两种：`SessionStart` 缺交互模式才带的键（`src/adapter/claude.rs` 的 `INTERACTION_ONLY_KEYS`）→ `headless`（宿主自己起的 `claude -p` / 子代理，裁决 agora-5gg.7 选 B），否则 `external`；判据只在这一条 `SessionStart` 上问一次，录不出结构差异的宿主（`codex exec`）不猜、照常按 `external` 登记。`Origin::is_handleless()` 是"无句柄那两档"的唯一判据：探活、supersede、归档重建、`ended_at` 都问它，headless 与 external 在这些地方同形；不同只有三处（不通知、折叠、到期不论状态），口径见 `docs/spec/api.md`「外部会话」与守卫 `tests/hooks_external.rs::a_headless_session_registers_as_headless_and_expires_whatever_its_status_is`（agora-5gg.20） | 只送来 SessionEnd 的会话是 agora 没见过的会话在结束——hook 装好之前起的会话退出、Codex Desktop 结束一个线程；登记只会造一行永远没有后续事件的僵尸（2026-09-05 侧栏那行 devcenter，agora-vfi） |
| 不登记之后 | 流程不变：id 为 None 就不 apply，`release_for` 仍按 `<host>:<agent_session_id>` 解挂起，文件进 done，账本记一条 `received` | 排障看得见它来过 |
| 身份交接删行 | 已登记的**无句柄 external 行**在状态机还没见过第一条 `PromptSubmitted` 时收到 `SessionEnd(reason = resume \| clear)` → 不 apply，直接 `delete_metadata`（检查点一起删）、文件进 done、返回 `Ok(None)`（行都没了，账本也不记谁的账）；删不动（库出错）才退回普通结束。判据走 `SessionManager::hook_prompt_seen`，宿主注入的 `prompt.injected` 不算人话（agora-3s5）。两侧都不越界：有 `runtime_ref` 的行不碰（身份是 pane，SessionEnd 之后同一进程再发的 SessionStart 落回同一行），有过 prompt 的 external 行也不碰（那是真结束，走上面 `clear_ends_external_row` 的改写）。守卫 `tests/hooks_external.rs::a_resume_handoff_before_the_first_prompt_leaves_no_row` | `claude --resume` 先以**新 id** 发 `SessionStart(source=startup)`，`locate_external` 查不到就登记出一行，12 s 后 `SessionEnd(reason=resume)` 落到它身上——旧代码留下一行无 prompt、无输出的 FINISHED 挂在侧栏，谁也没起过它（2026-09-18 zuan 现场 e3cd17，`docs/analysis/session-status-audit-2026-09-18.md` §3.4）。无句柄行的身份是 agent id，换 id 之后这个新行注定是空壳：删行 = 让它像从没被登记过，求差器在同一 tick 发 `session_removed` |
| 活性 | external 行的存活看 `AgentHooks::agent_pid` 给的进程号 `kill(pid, 0)`：Claude 用信封里的 `CLAUDE_PID`，Codex / Grok 用 hook 的 ppid（安装命令 `exec` 进 agora）。进程号与当时读到的进程启动时刻（macOS `proc_pidinfo`、Linux `/proc/<pid>/stat`）随 hook 检查点落盘（`HookSnapshot.agent_process`，v2），daemon 重启时回填 `SessionManager.external_pids`；探活时启动时刻对不上就是号被复用、原进程没了（agora-tql，2026-09-08 现场：重启后表空、11 行 external 钉在 TURN_DONE）。没有可信进程号（`Liveness::Unknown`）的行 hook 沉默超过 `hooks.external_silent_after` → UNKNOWN `hooks silent; no process handle`（`Machine::observe` 第 1 步；Idle / DecisionResolved 在它身上与屏幕给的 UNKNOWN 同样可信）。进程没了的行报 FINISHED（source process、conf 0.8、reason `external process gone (no exit status)`）——但只在 hook 没先说结束时：行已是 hook 的 FINISHED（`session ended (hook)` / `superseded: …`）的，进程消失只把 `alive` 变假，source / reason / `status_since` 都留 hook 的（`Machine::process_fact_is_no_better`：同状态、把握不比 hook 大的进程事实不盖 hook）。所以 external 行上 `session ended (hook)` = 宿主走完了退出流程（人在提示符退出、或关窗口时宿主还来得及发），`external process gone` = 一个事件都没来、只剩探活（Codex 关窗口、崩溃、机器重启）；宿主退出时发不发 SessionEnd 的对照表在 `src/adapter/{claude,codex,grok}.rs` 模块文档（agora-rzh，2026-09-08 现场：6 行探针 5 行发了 SessionEnd，行上却全是 process gone；守卫 `tests/hooks_external.rs::hook_session_end_survives_the_agent_process_going_away`、`tests/state_machine.rs::process_gone_does_not_overwrite_the_hook_session_end`）。 一个 CLI agent 进程一次只跑一个对话：同一宿主的两个无句柄 external 行报来同一个进程号，`seen_at` 早的那行被取代——合成 `AgoraEvent::Superseded` → FINISHED(hook) `superseded: …`（`SessionManager::supersede_external_rows`，登记新 external 行后与启动重放完各跑一次）。Grok 的 /clear、Codex TUI 的 /new 换 id 不发 SessionEnd，旧行会带着活着的进程号永远 TURN_DONE（2026-09-08 现场：一个 Grok 进程占了三行）；Claude 的 /clear 发（s3r 已处理，再落一次无害）。守卫 `tests/hooks_external.rs::a_new_conversation_on_the_same_process_supersedes_the_old_external_row`。**Codex Desktop 的 ppid 不可信**——信封 `agent_env` 带 `CODEX_INTERNAL_ORIGINATOR_OVERRIDE`（真投递件的值是 `Codex Desktop`；CLI 不设它）时 `agent_pid` 返回 None → `Liveness::Unknown`，行只跟 hook 走 | Desktop 线程的父进程是所有线程共用的常驻 `codex app-server`，探活永远为真；宁可不知道活不活，不要拿一个永远活着的进程当会话活着 |
| 挂起解除 | `ingest_inner` 尾部按 `release_for`：同工具的 PostToolUse 等 → `resolve(…, "terminal")`，Stop / SessionEnd → `resolve_session(…, "session")`；`sweep`（每 5 s）另管三种：超时 → `timeout`、**进程已退出**（`Receiver::sweep` 里的 `process_exited`：`process == gone` **且** `end_cause` 不是 `host_session_end` / `superseded`——三态之后单看 `gone` 不够，它把「无可信进程号（unknown）」与 Q4 的「对话结束即不再谈进程」都算作 gone，那两种都不是进程退了，2026-09-22 agora-qmi）→ `exit`、**状态机里已经没有这个键的 hold**（登记 ≥ 2 s、同一 epoch）→ `terminal`。最后一种的来源是状态机的"提示消失"规则（`src/status/machine.rs` 的 `observe_hooked`：有挂起、hook 给的 WAITING、文本层见过提示、之后连续 `text_ticks` 个 tick 见不到 → 清挂起、UNKNOWN(text)，钉住到下一条 hook 事件写出新状态；PostToolUse 的解除事件把它抬回 RUNNING，Notification(idle_prompt) 把它落到 TURN_DONE(hook, `idle`)——Esc 之后 Claude 停在提示符约 60 s 就发它，2026-09-06 agora-01g；沉默规则的 UNKNOWN 同样，两条共用 `unknown_from_screen` 谓词；挂起还在的 WAITING 收到 idle_prompt 仍不动）与 PromptSubmitted 清挂起。SessionManager 只在有挂起时对 hooked 会话每 tick capture 屏幕，hook 键与状态机 `pending` 的键是同一个 `decision_key` | 终端里放行后按 Esc，Claude 2.1.261 一个事件都不发（`testdata/claude/2.1.261/hooks/interrupted.jsonl`），hook 会挂到 55 min 超时、Dashboard 的 Allow / Deny 一直在、行钉在 needs input；挂起期间 capture 是 ADR-002 D1 沉默规则的唯一例外，文本层在此只能降不能抬；落 UNKNOWN 不落 RUNNING，因为中断与工具正在跑在屏幕上分不出来（agora-9cd） |

状态机（`src/status/machine.rs`）对 `SessionEnded(reason)`：清挂起；`reason = clear`（Claude 的 /clear：进程活着，同一秒紧接着新 id 的 `SessionStart(source=clear)`）**不改状态**——前提是新 id 落回同一行（agora 起的会话、有 pane 的采纳行，`register_external` 按 runtime_ref 复用）；无句柄的 external 行以 agent id 为身份，新 id 另起一行，旧行从此没有事件，所以 receiver（`src/hook/receiver.rs` 的 `clear_ends_external_row`）在送进状态机之前把这种行的 `clear` 改写成别的 reason，按普通结束处理（2026-09-07 现场：/clear 之后旧行 working 16 min+，没有 pane 沉默规则够不着、CLAUDE_PID 还在跑新会话，agora-s3r；守卫 `tests/hooks_external.rs::external_row_ends_when_claude_clears_to_a_new_id`）；其余 reason（Claude resume / logout / prompt_input_exit / other、Codex other、Grok shutdown、没有 reason）→ **FINISHED**（source hook、conf 0.8、reason `session ended (hook)`）。带退出码的进程事实照旧以 1.0 覆盖（observe 第 1 步），所以 agora 起的会话不受影响；没有进程事实的 external 会话则靠这一条结束，下一 tick 就是 ✓。有进程号的 external 行随后探活到进程没了（同状态、同 0.8 的 `external process gone`）**不覆盖**它：source / reason / `status_since` 留 hook 的，只有 `alive` 变假——否则人自己在终端结束的与终端被关 / 崩溃的在行上混成一个词（agora-rzh；FAILED 或 conf 1.0 的进程事实仍覆盖，见上表「活性」）。FINISHED(hook) 不是终态：之后的 `SessionStart` 回 STARTING（Codex TUI 的 /new、Claude 的 /resume），只有进程层的 FINISHED / FAILED 才压倒一切。fixture 的 `expect` 随之改（`testdata/*/*/hooks/*.jsonl` 里 SessionEnd 之后的行是 `finished` / `hook`）。s3r 之前写下的检查点停在修复前的结论（clear 不改状态 → TURN_DONE），而去重水位会拒掉归档里那条 SessionEnd：daemon 启动恢复时，无句柄 external 行的 v1 检查点只要 done/ 里还有它的投递就丢掉检查点、从归档按序重建，重建走的同一条改写（`Receiver::restore_archive`，agora-tql）。

守卫：`tests/hooks_external.rs::session_end_for_an_unseen_session_registers_nothing`、`::external_session_ends_on_session_end_hook`、`::legacy_checkpoint_of_a_handleless_external_row_is_rebuilt_from_the_archive`；`tests/hook_recovery.rs::external_agent_pid_survives_daemon_restart_and_guards_pid_reuse`；`tests/state_machine.rs::handleless_external_silence_falls_to_unknown`；`tests/state_machine.rs::external_session_ended_by_hook_is_finished_not_unknown`、`::session_end_reason_clear_keeps_the_session_alive`、`::session_start_after_session_end_restarts`；`src/adapter/codex.rs` 单测 `codex_desktop_parent_is_not_the_agent_pid`；挂起解除那一行：`tests/state_machine_pending.rs::pending_permission_is_released_when_the_prompt_leaves_the_screen`、`::pending_permission_stays_while_the_prompt_is_on_screen`、`::pending_permission_never_seen_on_screen_is_left_alone`、`::dashboard_or_terminal_resolution_still_wins`、`::idle_notification_after_the_prompt_is_gone_lands_on_turn_done`、`::idle_notification_while_the_prompt_is_still_pending_is_ignored`、`tests/state_machine.rs::idle_notification_after_silent_hooks_unknown_lands_on_turn_done`、`tests/api_input.rs::an_interrupted_permission_is_released_via_terminal_when_the_prompt_disappears`。

external 行的 STARTING 同样带衰减（agora-rkl，2026-09-18 现场：zuan 的 `ef0e50` 只收到过一条 SessionStart，钉在 `… starting` 180 h、`a3a2a0` 10 h；Mac 的 `trion` 2 d、`dbs-operator` 15 h）：ADR-002 D1 那条「hook 层的 STARTING 在启动宽限（`STARTUP_GRACE_SECS` = 10 s）内没有后续 hook 事件 → TURN_DONE `session started, awaiting first prompt`」不分 origin（MISSION §4.3 的 STARTING 行写的就是"最多停 10 s"），但规则原先只写在 `observe_hooked` 里，而 external 行的进程层恒为 `Source::None`、`Machine::observe` 第 1 步就 return，对它们不可达；STARTING 于是没有出口（2 h 的沉默兜底只对 `Liveness::Unknown` 生效，进程号活着的那几行连兜底都轮不到）。修法不是拆掉第 1 步的 return（那会让 external 行落到"进程层 RUNNING"这种它们根本没有的事实上），而是在第 1 步的 external 分支里落同一个 `Machine::decay_starting`：`Liveness::Alive` 与 `Liveness::Unknown` 都衰减，`Dead` 不衰减——进程事实压倒 hook。它与同分支的沉默兜底共用一个 quiet 时钟，先后顺序不影响结论（兜底一旦落 UNKNOWN，STARTING 的门就关上不再开）。守卫：`tests/status_truth_table.rs`（状态真值表的第一行，其余组合由 agora-5gg.16 往里追加）、`tests/state_machine.rs::handleless_external_starting_decays_to_turn_done`、`tests/hook_recovery.rs::handleless_external_starting_decays_through_session_manager_and_after_checkpoint_restore`。

## 运行时会话没了：FINISHED，不是永远 UNKNOWN（agora-u5p）

有 `runtime_ref` 的行，运行时那一刻正常应答、而列表里没有这个会话——**结论是 FINISHED**（`source = process`、`conf = 0.8`、reason `runtime session gone (…; no exit status)`），不再是 2026-09-03 定的 UNKNOWN `runtime session missing`（ADR-001 D4 的修订记录与理由）。链路是：

| 环节 | 规则 | 为什么 |
|---|---|---|
| 判"没了" | `SessionManager::view` 里的 `gone`：`unreadable` 为空 **且** `live` 里没有这个 ref。`unreadable` 是"这一行的运行时读不到"，两种来路都算：整体降级（协议不匹配、超时、server 应答不了，ADR-001 D7），**或它所在的那个采纳 socket 这一次没扫成**（`Runtime::list()` 交出 per-socket 的扫描结果，agora-dkv3）。任一条成立就落 UNKNOWN `runtime unavailable: …` | "读不到"与"运行时答话说没有"是两件事；拿前者当后者会误写 `ended_at`、误报结束。采纳 socket 的失败在 `list()` 里只 warn（用户自己的 tmux 不得拖垮 agora 的会话列表），吞掉之后"列表里没有它"就分不开这两种读法 |
| 说"怎么没的" | `Runtime::server_present(ref)`（tmux 的实现是 connect 一下那个 unix socket，不起子进程）：连不上 → `runtime session gone (server gone; …)`；连得上 → `(session gone; …)`。注意它只在 `unreadable` 为空时轮得到：那个 socket 这次没扫成时根本不下 `gone` 的结论，也就无所谓 server 在不在 | 一屋子行同时结束（daemon 停几天后 server 没了）与单个会话被 `kill-session` 是两回事，排障时这一句就是答案 |
...
守卫：`tests/session_manager.rs::missing_runtime_session_finishes_the_row_and_writes_ended_at_once`、`::server_gone_and_session_gone_are_told_apart_in_the_reason`、`::runtime_gone_of_a_killed_row_still_says_killed_by_user`、`::degraded_runtime_never_becomes_runtime_gone`、`::runtime_gone_notifies_once_and_obeys_killed_by_user`、`::reconcile_covers_all_six_cases`（missing 一格改断 FINISHED）；采纳 socket "没扫到 ≠ 会话没了"（两条 missing 分支各一条，agora-dkv3）在 `tests/runtime_degraded.rs::unscanned_adopt_socket_keeps_adopted_rows_unknown` 与 `::reconcile_skips_rows_on_an_unscanned_adopt_socket`；真实 tmux 的复现（起 shell → `tmux -L <socket> kill-server` → 下一 tick FINISHED + Restart 把 server 带回来）在 `tests/session_tmux.rs::kill_server_finishes_every_row_on_it`；状态机层在 `tests/state_machine.rs::runtime_session_gone_finishes_the_row_instead_of_pin_it_at_unknown`；真值表在 `tests/status_truth_table.rs::runtime_session_gone_is_a_fact_with_an_exit_not_a_dead_end`（session gone / server gone / killed by user / 降级四行各钉一格）与 `::runtime_session_gone_does_not_outvote_a_hook_that_spoke_first`（抬到 1.0 就红）；前端在 `web/src/TerminalView.test.tsx` 的 `TerminalView when the runtime session is gone`；与进程三态的接缝（这一行 `process = gone`、降级那格 `unknown`）钉在上面 `tests/session_manager.rs` 两条里的 `process` 断言。
