...
4. 重启 daemon（`--no-restart` 跳过，"daemon 下次启动即新版本"），按顺序探测、命中即用：
   ...
   2. macOS 上 `launchctl print gui/<uid>/dev.agora.daemon` 成功 **且** 输出里 `program = ` 是本 `<AGORA_HOME>/bin/agora` → `launchctl kickstart -k gui/<uid>/dev.agora.daemon`（同一个理由；`program` 行认不出时按装载算）；
