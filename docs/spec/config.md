...
hooks:                        # ADR-002 D1 / D5 / D3
  ...
  external_silent_after: "2h" # 没有可信进程号的 external 行（Codex Desktop、旧检查点）hook 沉默这么久 → UNKNOWN（agora-tql）
...
macOS 那一支在 Linux 上也能跑：环境变量 **`AGORA_INSTALL_OS=Darwin|Linux`** 强制脚本按某个系统走（真机安装不要设，设错会把单元写到 `~/Library/LaunchAgents` 再抱怨没有 `launchctl`），配一个只把 argv 记到日志的假 `launchctl` 就能断言单元内容与 `launchctl` 的调用序列（`macos_branch_writes_launchd_unit_and_bootstraps_it` 等六条，agora-5gg.15）；钩子本身也钉住：取值只能是这两个之一，设成第三个值就拒绝且错误信息点名 `AGORA_INSTALL_OS`（`agora_install_os_only_lets_you_pretend_to_be_a_supported_os`——只说「不支持的系统: FreeBSD」会把人推向「那是不是该加个 FreeBSD 分支」）。为什么要这个钩子：CI 的 matrix 里有 `macos-latest`，macOS 那一支本来就会跑，可旧有的守卫一律带 `--no-service`，`launchctl` 那半段（print / bootstrap / bootout）在任何机器上都没被执行过。假 launchctl 只钉「脚本怎么叫 launchctl」，launchd 真能把进程拉起仍得在真 Mac 上看（见下面 macOS 一支的末句）。
...
4. **随登录自启**，单元里写 `LANG=C.UTF-8`（devcenter CJK 乱码教训的另一半，ADR-001 D7）、`AGORA_HOME`、够找到 tmux 与二进制的 `PATH`（`~/.local/bin:/usr/local/bin:/usr/bin:/bin`，macOS 多一个 `/opt/homebrew/bin`；tmux 装在这些目录之外就把它所在目录也加上；完整 PATH 由 daemon 启动时自己探测）。内容没变就不重写文件。
   - Linux：`~/.config/systemd/user/agora.service`（模板 `scripts/templates/agora.service`：`ExecStart=<home>/bin/agora serve`、`Restart=on-failure`、`WantedBy=default.target`、**`KillMode=process`**——systemd 默认的 `control-group` 会在 stop / restart 时把 daemon 起的 tmux server 与全部 agent 一起杀，2026-09-07 zuan 实测 reconcile `missing=3`（agora-x1z；不变量 3、A39）；`process` 只对主进程发 SIGTERM，tmux server 留在 cgroup 里等新 daemon reconcile。守卫 `tests/install_script.rs` 在 Linux 上断言这一行），非 `--no-service` 时 `systemctl --user daemon-reload && systemctl --user enable --now agora.service`；没有 `systemctl`（容器）只写文件、说一句、不报错。**linger**：`loginctl show-user $USER -p Linger` 不是 `yes` 就提示 `sudo loginctl enable-linger $USER`（用户单元随登录起、随登出停，不开 linger 重启后要等人登录 daemon 才起）——脚本不自己 sudo，装软件与改系统设置的每一步都由人敲。`/etc/systemd/logind.conf`（含 `logind.conf.d/*.conf`）里 `KillUserProcesses=yes` 时警告：登出会杀掉 daemon 与 tmux server。
   - macOS：`~/Library/LaunchAgents/dev.agora.daemon.plist`（label `dev.agora.daemon`，模板 `scripts/templates/dev.agora.daemon.plist`：`ProgramArguments <home>/bin/agora serve`、`RunAtLoad`、`KeepAlive` 仅 `SuccessfulExit=false`——非零退出才拉起，等价 `Restart=on-failure`；stdout / stderr 落 `<home>/daemon.log`）。非 `--no-service` 时先用 `launchctl print gui/$(id -u)/dev.agora.daemon` 探状态：**没加载**才 `launchctl bootstrap`；**已加载且单元内容没变**就一个命令都不下（重跑安装不该把 daemon 抽掉、打断正在等的会话，也不该凭空造一次全量重放：2026-09-18 Mac 恢复时一次重放 14026 件、172 s）；**已加载但内容变了**要 `bootout` 再 `bootstrap`——`kickstart -k` 只把当前已加载的那份定义重启一遍，不重读磁盘上的 plist，拿它当「让它读新单元」会让人以为新版已生效而实际跑的还是旧 PATH。`bootstrap` 失败（从 ssh 装时 `gui` 域常常不在，报 `Bootstrap failed: 5`）会先打出 `sudo launchctl asuser $(id -u) launchctl bootstrap …` 与「到 Mac 本机开终端」两条命令，再非零退出——脚本不自己 sudo，改系统设置的每一步由人敲。守卫：`macos_branch_writes_launchd_unit_and_bootstraps_it`（写单元 + bootstrap 装载）、`macos_dry_run_and_no_service_never_call_launchctl`（dry-run / `--no-service` 不弄真命令）、`macos_rerun_with_unchanged_unit_leaves_daemon_alone`（重跑不动服务）、`macos_changed_unit_is_reloaded_with_bootout_then_bootstrap`、`macos_bootstrap_failure_prints_the_manual_command`、`macos_warns_when_an_unmanaged_daemon_holds_the_home`（launchd 说单元没装载而 `<home>/agora.pid` 里的进程还活着 = 这台手工 daemon 不受监督且占着端口，bootstrap 之后 launchd 起的那一个会因端口被占反复重试（默认节流 10 s）：打出 `kill <pid>` 与 `launchctl print` 两条命令，只警告不拒装——本任务要修的 D1 就是这个形状）。launchd 自身的动作在 Linux CI 上测不到，「装好后杀掉 daemon 能自动拉起」归人眼（agora-5gg 验收 (c)）。
...
**幂等**：重跑退出 0，config.yaml / versions 里的二进制 / 单元文件 mtime 与内容都不变，链接不重做（守卫 `install_writes_config_link_and_unit_idempotently`）；服务侧同样不折腾——已加载且单元内容没变就不重启 daemon（守卫 `macos_rerun_with_unchanged_unit_leaves_daemon_alone`；Linux 侧 `enable --now` 对已 active 的单元本就是空操作）。**输出纪律**：给人看的话一律走 stderr，stdout 留给将来机器可读的输出（MISSION §2.3 规则 10）。
