...
| agent | 本机装有 Claude Code 2.1.258、Codex CLI 0.152.1、Grok 1.0.13（`~/.grok/bin/grok`）、cursor-agent（版本 2026-09-02 以 `--version` 复核；9-01 记的 Codex 0.145.0 是过期的更新检查记录）；pi 未装；zuan 上尚无任何 agent |
| 运行时 | 两台机器都可按需安装配置（tmux / Docker / 容器 / SDK 均可），运行时选择不受现状约束（ADR-001） |
| 网络 | Mac（`rickys-macbook-air`）与 iPhone（`iphone171`）已在 tailnet（`tail6f613.ts.net`）；zuan 待装 tailscale；Android 待加入（tailnet 里只有一台 3 年未上线的 `pro`）；tailnet 内还有他人共享的节点与 tagged 设备（ADR-003 拒绝启动守卫的动机之一）；zuan 出网：Apple 推送 / Let's Encrypt / tailscale 可达，**FCM 不可达**（MISSION §6.6 降级策略的来源） |
