...
（RUNNING 段最后那行是**看过一次**的 TURN_DONE：选中展开过一次就从中段继续待着，等下一次 TURN_DONE 换个 `status_since` 再回 NEEDS ATTENTION，MISSION §4.6 证据 ①、agora-5gg.21。它不在 `▸ FINISHED` 里——那一区的行是「Finished N」一键清理的删除对象，而这行的进程还活着。）
